...
function studentValidate(sID: number) {
  let exist = false;
...
function courseValidate(ind: number, selCourse: string) {
...
function balanceValidate(ind: number, cBalance: number) {
...
async function enrolStudent() {
  ...
  const selectedCourse = course_input.coursName as keyof typeof courses;
